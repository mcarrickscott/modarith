...
// Use associated python scripts (pseudo_cuda.py or monty_cuda.py) to generate code for X25519 or X448, but easily modified for other Montgomery curves
//
// Mike Scott 13th October 2025
// TII
//
// field.cu code for X25519 curve can be generated by 
//
// python pseudo_cuda.py X25519
//
// field.cu code for X448 curve can be generated by
//
// python monty_cuda.py X448
//
// where XXX can be neon, sse or ifma
//
// make sure decoration and generic are both set to False
// 
// Needs Nvidia hardware and software development tools
// Tested on GeForce MTX570a
// nvcc -O2  rfc7748_simt.cu -lcpucycles -o rfc7748

/*** Insert automatically generated code for modulus field.cu here ***/
...
__device__ static void reverse(char *w)
{
    int i;
...
/*
__device__ static void output(spint *x) {
    char b[Nbytes+1];
    char buff[(2*Nbytes)+1];
    modexp(x,b);  // first lane only
    toHex(b,buff);
    puts(buff);
}
*/
...
__device__ static void clamp(char *bk) {
    int s=(8-(Nbits%8))%8;
    bk[0]&=-(1<<COF);
...
__device__ static int bit(int n,const char *a) {
    return (int)((a[n/8u]&((unsigned char)1u<<(n%8u)))>>(n%8u));
}

__device__ static char mask() {
    int r=Nbits%8;
    if (r==0) r=8;
    return (char)((1<<r)-1);
}
...
__global__ void rfc7748(const char *bk, const char *bu,char *bv) {
    int i,n=blockIdx.x;
    int swap,sb;
    char ck[Nbytes];
    char cu[Nbytes];
...
// consider only 2 simultaneous calculations, so n=0 or 1. Avoid branching.
    for (i=0;i<Nbytes;i++) {
        ck[i]=bk[i+Nbytes*n];
        cu[i]=bu[i+Nbytes*n];
    }
...
// convert to byte array
    ...
    char *d_bk, *d_bu, *d_bv;
    cudaMalloc(&d_bk, 2*Nbytes);
    cudaMalloc(&d_bu, 2*Nbytes);
    cudaMalloc(&d_bv, 2*Nbytes);    
    
    cudaMemcpy(d_bk, bk1, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(&d_bk[Nbytes], bk2, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(d_bu, bu1, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(&d_bu[Nbytes], bu2, Nbytes, cudaMemcpyHostToDevice);

    rfc7748<<<2,1>>>(d_bk,d_bu,d_bv);

    cudaMemcpy(bv1, d_bv, Nbytes, cudaMemcpyDeviceToHost);
    cudaMemcpy(bv2, &d_bv[Nbytes], Nbytes, cudaMemcpyDeviceToHost);    
...
// convert to Hex
    ...
    fin=cpucycles();
#endif
    printf("Host clock cycles per point multiplication= %d\n",(int)((fin-start)/20000ULL));
#endif

    cudaMemcpy(bu1, d_bu, Nbytes, cudaMemcpyDeviceToHost);
    cudaMemcpy(bu2, &d_bu[Nbytes], Nbytes, cudaMemcpyDeviceToHost);  
...
// do a double D-H key exchange
    ...
    char *d_alice, *d_apk, *d_bob, *d_bpk, *d_ssa, *d_ssb;
    cudaMalloc(&d_alice, 2*Nbytes);
    cudaMalloc(&d_apk, 2*Nbytes); 
    cudaMalloc(&d_bob, 2*Nbytes);
    cudaMalloc(&d_bpk, 2*Nbytes); 
    cudaMalloc(&d_ssa, 2*Nbytes);
    cudaMalloc(&d_ssb, 2*Nbytes); 

    cudaMemcpy(d_alice, alice1, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(&d_alice[Nbytes], alice2, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(d_apk, apk1, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(&d_apk[Nbytes], apk2, Nbytes, cudaMemcpyHostToDevice);

    cudaMemcpy(d_bob, bob1, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(&d_bob[Nbytes], bob2, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(d_bpk, bpk1, Nbytes, cudaMemcpyHostToDevice);
    cudaMemcpy(&d_bpk[Nbytes], bpk2, Nbytes, cudaMemcpyHostToDevice);

    rfc7748<<<2,1>>>(d_alice,d_apk,d_apk);
    rfc7748<<<2,1>>>(d_bob,d_bpk,d_bpk);

    rfc7748<<<2,1>>>(d_alice,d_bpk,d_ssa);
    rfc7748<<<2,1>>>(d_bob,d_apk,d_ssb);
    cudaDeviceSynchronize();

    cudaMemcpy(ssa1, d_ssa, Nbytes, cudaMemcpyDeviceToHost);
    cudaMemcpy(ssa2, &d_ssa[Nbytes], Nbytes, cudaMemcpyDeviceToHost);  
    cudaMemcpy(ssb1, d_ssb, Nbytes, cudaMemcpyDeviceToHost);
    cudaMemcpy(ssb2, &d_ssb[Nbytes], Nbytes, cudaMemcpyDeviceToHost); 
