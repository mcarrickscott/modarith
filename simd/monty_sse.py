...
def intrinsics() :
    ...
    str+="static inline spint _mm_set2_epi32(int c) {\n"
    str+="\treturn _mm_set_epi32(0,c,0,c);\n"
    str+="}\n" 
    str+="// set each lane to a constant\n"
    str+="static inline spint _mm_setc_epi32(int c0,int c1) {\n"
    str+="\treturn _mm_set_epi32(0,c1,0,c0);\n"
    str+="}\n" 
    str+="// t+=a*b where t is 64 bits, a and b 32 bits\n"
    str+="static inline spint _mm_mla_epu32(spint t,spint a,spint b) {\n"
    str+="\tspint pp=_mm_mul_epu32(a,b);\n"
    str+="\treturn _mm_add_epi64(t,pp);\n" 
    str+="}\n" 

    str+="// t+=a*b where t is 64 bits, a and b 32 bits\n"
    str+="static inline spint _mm_mla_epi32(spint t,spint a,spint b) {\n"
    str+="\tspint pp=_mm_mul_epi32(a,b);\n"
    str+="\treturn _mm_add_epi64(t,pp);\n" 
    str+="}\n" 

    str+="// t=a*c where t, a is 64 bits and c is a small constant\n"
    str+="static inline spint _mm_mlc_epu64(spint a,int c) {\n"
    str+="\tspint s=_mm_set2_epi32(c);\n"
    str+="\tspint pp1=_mm_mul_epu32(a,s);\n"
    str+="\tspint pp2=_mm_mul_epu32(_mm_srli_epi64(a,32),s);\n"
    str+="\treturn _mm_add_epi64(pp1,_mm_slli_epi64(pp2,32));\n"
    str+="}\n" 
    str+="// t=a*c where a is 32 bits and c is a constant\n"
    str+="static inline spint _mm_mlc_epu32(spint a,int c) {\n"
    str+="\tspint s=_mm_set2_epi32(c);\n"   
    str+="\treturn _mm_mul_epu32(a,s);\n"
    str+="}\n"     
    str+="// t+=a*c where a is 32 bits and c is a constant\n"
    str+="static inline spint _mm_mlca_epu32(spint t,spint a,int c) {\n"
    str+="\tspint s=_mm_set2_epi32(c);\n"
...
def modmli(n) :
    ...
    str+="\tspint mask=_mm_set2_epi32((1<<{}u)-1);\n".format(base)
    str+="\tudpint t=_mm_setzero_si128();\n"
...
#        str+="/*\n"
        ...
        str+="\tt=_mm_mla_epu32(t,a[{}],bw); ".format(N-1)
        str+="c[{}]=(spint)t;\n".format(N-1)
...
def modcsw() :
    ...
    if PSCR :
        ...
        str+="\tspint zero=_mm_setzero_si128();\n"
        str+="\tspint delta,mask=_mm_sub_epi32(zero,b);\n"
        str+="\tfor (i=0;i<{};i++) {{\n".format(N)
