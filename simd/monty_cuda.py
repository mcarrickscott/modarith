# Python program to generate reasonably efficient C/C++ modular arithmetic code for any prime using Nvidia CUDA GPUs
...
def prop(n) :
    ...
    str+="__device__ static spint inline prop(spint *n) {\n"
    str+="\tint i;\n"
...
def time_modmul(n,ra,rb) :
    ...
    str="__global__ void time_modmul{}() {{\n".format(DECOR)
    str+="\tspint x[{}],y[{}],z[{}];\n".format(N,N,N)
    str+="\tint i,j,n=blockIdx.x;\n"
    str+="\tlong long int begin,end,elapsed;\n"
    str+="\t"
    for i in range(0,N) :
        str+="x[{}]=n+{}; ".format(i,hex(rap[i]))
    str+="\n\t"
    for i in range(0,N) :
        str+="y[{}]=n+{}; ".format(i,hex(rbp[i]))
    str+="\n"
    ...
    str+="\tend=clock64();\n"
    str+="\telapsed = (end - begin)/1000000;\n"
    str+="\tredc{}(z,z);\n".format(DECOR)
    str+='\tprintf("%d. modmul check 0x%06x Clock cycles= %lld\\n",n,(int)z[0]&0xFFFFFF,elapsed);\n'
    str+="}\n"
    return str
...
def time_modsqr(n,r) :
    ...
    smask=(1<<bit)-1
    rp=makebig(r,base,N)
    str="__global__ void time_modsqr{}() {{\n".format(DECOR)
    str+="\tspint x[{}],z[{}];\n".format(N,N)
    str+="\tint i,j,n=blockIdx.x;\n"
    str+="\tlong long int begin,end,elapsed;\n"
    str+="\t"
    for i in range(0,N) :
        str+="x[{}]=n+{}; ".format(i,hex(rp[i]))
    str+="\n"
    ...
    str+="\telapsed = (end - begin)/1000000;\n"
    str+="\tredc{}(z,z);\n".format(DECOR)
    str+='\tprintf("%d. modsqr check 0x%06x Clock cycles= %lld\\n",n,(int)z[0]&0xFFFFFF,elapsed);\n'
    str+="}\n"
    return str
...
def time_modinv(n,r) :
    ...
    smask=(1<<bit)-1
    rp=makebig(r,base,N)
    str="__global__ void time_modinv{}() {{\n".format(DECOR)
    str+="\tspint x[{}],z[{}];\n".format(N,N)
    str+="\tint i,n=blockIdx.x;\n"
    str+="\tlong long int begin,end,elapsed;\n"
    str+="\t"
    for i in range(0,N) :
        str+="x[{}]=n+{}; ".format(i,hex(rp[i]))
    ...
    str+="\t}\n"
    str+="\tend=clock64();\n"
    str+="\telapsed = (end - begin)/1000;\n"
    str+="\tredc{}(z,z);\n".format(DECOR)
    str+='\tprintf("%d. modinv check 0x%06x Clock cycles= %lld\\n",n,(int)z[0]&0xFFFFFF,elapsed);\n'
    str+="}\n"
    return str
...
if len(sys.argv)!=2 :
    print("Syntax error")
    print("Valid syntax - python monty_cuda.py <prime> OR <prime name>")
    print("For example - python monty_cuda.py NIST256")
    print("For example - python monty_cuda.py 0x01fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffa51868783bf2f966b7fcc0148f709a5d03bb5c9b8899c47aebb6fb71e91386409")
    exit(2)
