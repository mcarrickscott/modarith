...
// Input private key
// Output public key
void XXX_KEYGEN(char *prv,char *pub)
{
    point G;
...
// Methods are
// 1. non-deterministic - a non-biased high entropy random number in the range from 0 - q (the group order) - recommended is CSRNG is good
...
// Output signature.
void XXX_SIGN(char *prv,char *rp,char *m,char *sig)
{
    point G;
...
int XXX_VERIFY(char *pub,char *m,char *sig) 
{
    point G;
    gel r,s;     // some group elements..
    ecngen(&G);  // get curve generator point
