# Python program to generate reasonably efficient C/C++ modular arithmetic code for pseudo-mersenne primes on a 32 or 64-bit processor
...
# The radix for an n-bit prime is 2^base where base is usually in the range 51-60 for a 64-bit processor, and 26-30 for 32-bit
# Radix must be low enough so that the sum of L 2^(2*base) double length integers does not encroach on the sign bit, 
...
#if m>=1024 : # bits(m)+base>=2*WL-1 :   3 ********
#    print("Not a good pseudo-Mersenne - Use Montgomery method instead")
#    exit(0)
